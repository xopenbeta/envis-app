...
/// 启动 GUI 应用。
///
/// `ctx` 由主 crate 通过 `tauri::generate_context!()` 生成并传入，
/// 因为 `generate_context!()` 必须在含有 `tauri.conf.json` 的 crate（envis 主包）中调用。
pub fn run(ctx: tauri::Context<tauri::Wry>) {
    ...
    let app = tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            list_postgresql_roles,
            create_postgresql_role,
            delete_postgresql_role,
            update_postgresql_role_grants,
            // Python 服务命令
            download_python,
            get_python_versions,
