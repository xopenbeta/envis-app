...
use super::{GradleService, MavenService};
...
impl JavaService {
    ...
    pub fn set_maven_local_repository_to_settings(
        ...
        MavenService::global().set_maven_local_repository_to_settings(java_version, local_repo)
    }

    // ─── Gradle 代理方法 ──────────────────────────────────────────────────────

    /// 检查 Gradle 是否已安装（代理方法）
    pub fn is_gradle_installed(&self, java_version: &str) -> bool {
        GradleService::global().is_gradle_installed(java_version)
    }

    /// 获取 Gradle 安装目录（代理方法）
    pub fn get_gradle_home(&self, java_version: &str) -> Option<String> {
        GradleService::global().get_gradle_home(java_version)
    }

    /// 下载并安装 Gradle（代理方法）
    pub async fn download_and_install_gradle(&self, java_version: &str) -> Result<DownloadResult> {
        GradleService::global()
            .download_and_install_gradle(java_version)
            .await
    }

    /// 获取 Gradle 下载进度（代理方法）
    pub fn get_gradle_download_progress(&self, java_version: &str) -> Option<DownloadTask> {
        GradleService::global().get_gradle_download_progress(java_version)
    }
}
...
/// 解压 tar 格式文件
pub(crate) async fn extract_tar(archive_path: &PathBuf, target_dir: &PathBuf) -> Result<()> {
...
/// 设置可执行权限
#[cfg(not(target_os = "windows"))]
pub(crate) fn set_executable_permissions(install_dir: &PathBuf) -> Result<()> {
    use std::os::unix::fs::PermissionsExt;
